...
func (s *Service) RunAction(ctx context.Context, action *domain.Action, release *domain.Release) (rejections []string, err error) {
	...
	case domain.ActionTypeWebhook:
		rejections, err = s.webhook(ctx, action)
...
func (s *Service) webhook(ctx context.Context, action *domain.Action) ([]string, error) {
	l := zerolog.Ctx(ctx)
	...
	if len(body) > 0 {
		...
		if jsonErr := json.Unmarshal(body, &parsed); jsonErr == nil {
			...
			if rejected || explicitNotApproved {
				...
				l.Debug().Str("host", action.WebhookHost).Strs("rejections", reasons).Dur("duration", time.Since(start)).Msg("webhook rejected release")
				return reasons, nil
			}
		}
	}
